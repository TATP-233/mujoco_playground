...
from mujoco_playground._src.manipulation.universal_robots_ur5e_robotiq import ur5e
...
class Ur5ePickCube(ur5e.Ur5eRobotiqBase):
  ...
  def _post_init(self, obj_name: str, keyframe: str):
    all_joints = ur5e._ARM_JOINTS + ur5e._FINGER_JOINTS
    self._robot_arm_qposadr = np.array([
        self._mj_model.jnt_qposadr[self._mj_model.joint(j).id]
        for j in ur5e._ARM_JOINTS
    ])
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    target_pos = (
        jax.random.uniform(
            rng_target,
            (3,),
            minval=jp.array([-0.15, -0.15, 0.1]),
            maxval=jp.array([ 0.15, 0.15, 0.2]),
        )
        + self._init_obj_pos
    )

...
class ArmKdl:
    ...
    def __init__(
        ...
        self.punish = np.array(punish if punish is not None else [1.0] * 6)
    ...
    def _cut_end(self, pose: np.ndarray) -> np.ndarray:
        ...
        res_rot = rot
        res = np.eye(4)
        res = res.at[0:3, 0:3].set(res_rot)
        res = res.at[0:3, 3].set(res_pos)
        res = res.at[3, 3].set(1.0)
...
def main():
    ...
    """
    from airdc.common.utils.tf import matrix2pose, pose2matrix
    for eef_type in ["E2B"]:  # "none", "G2",
        arm_type = "play_short"
        arm_kdl = ArmKdl(arm_type, eef_type)
        print(f"Arm type: {arm_type}, EEF type: {eef_type}")
        # print("DH parameters:")
        # print("a:", arm_kdl.dh.a)
        # print("d:", arm_kdl.dh.d)
        # print("alpha:", arm_kdl.dh.alpha)
        # print("theta_offset:", arm_kdl.dh.theta_offset)
        # print()
        js = [0.01895605, -1.1606754 ,  0.9371874 ,  1.3908561 , -1.0946779 ,
       -1.6399724]
        print("joints")
        print(js)
        end_pose = arm_kdl.forward_kinematics(np.asarray(js))
        print("end_pose")
        print(end_pose)
        # print(f"{matrix2pose(end_pose)=}")
    # print(

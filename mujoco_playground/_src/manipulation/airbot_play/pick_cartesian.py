...
from mujoco_playground._src.manipulation.airbot_play import pick
from mujoco_playground._src.manipulation.airbot_play.kdl.arm_kdl import ArmKdl
...
class AirbotPlayPickCubeCartesian(pick.AirbotPlayPickCube):
  ...
  def __init__(
    ...
    self._arm_kdl = ArmKdl("play_short", "G2")
    self._start_tip_transform = self._arm_kdl.forward_kinematics(
        self._init_ctrl[:6]
    )
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    #     + self._init_obj_pos
    # )
    # NOTE: the same as the picked key
    target_pos = jp.array([0.294, 0.0, 0.11])
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    lifted = (box_pos[2] > (self._init_obj_pos[2] + 0.01)) * self._config.reward_config.lifted_reward
    total_reward += lifted
  ...
  def _move_tip(
    ...
    out_jp = self._arm_kdl.inverse_kinematics(
        new_tip_mat, current_ctrl[:6]
    )[0]
    no_soln = jp.any(jp.isnan(out_jp))
    out_jp = jp.where(no_soln, current_ctrl[:6], out_jp)

...
class AirbotPlayPickCubeCartesian(airbot_play.AirbotPlayBase):
  ...
  def __init__(
    ...
    self._ik_data = mujoco.MjData(self._mj_model)
    self._ik_data.qpos[:] = self._init_q
    mujoco.mj_forward(self._mj_model, self._ik_data)
    self._ik_solver = MinkIK(
        self._mj_model,
    ...
    if hasattr(self._ik_solver, "posture_task"):
      self._ik_solver.posture_task.set_target_from_configuration(
          self._ik_solver.configuration
      )
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    # First three action dims: end-effector xyz delta in world frame.
    cart_delta = action[:3] * self._action_scale
    target_pos = data_fk.site_xpos[self._gripper_site] + cart_delta
  
    # Use pure_callback to execute Host-side IK solver from within JIT-compiled step
    arm_ctrl = jax.pure_callback(
        self._solve_cartesian_ik,
        jax.ShapeDtypeStruct((self._robot_arm_qposadr.size,), jp.float32),
        data_fk.qpos,
        target_pos,
    )
    ctrl = state.data.ctrl
    ctrl = ctrl.at[: self._robot_arm_qposadr.size].set(
        jp.clip(
            arm_ctrl,
            self._lowers[: self._robot_arm_qposadr.size],
            self._uppers[: self._robot_arm_qposadr.size],
        )
    )
  
    gripper_delta = action[-2:] * self._action_scale
    if self._vision:
        gripper_delta = jp.where(gripper_delta < 0, -1.0, 1.0) * 0.02  # up to 2 cm movement per ctrl.
    gripper_ctrl = jp.clip(
        ctrl[-2:] + gripper_delta,
        self._lowers[-2:],
        self._uppers[-2:],
    )
    ctrl = ctrl.at[-2:].set(gripper_ctrl)
